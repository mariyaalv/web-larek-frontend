...
export class FormPayment extends Form<TFormOfPayment> implements IFormOfPayment {
  ...
  constructor(protected container: HTMLFormElement, events: IEvents) {
    ...
    this.inputAddress = this.container.elements.namedItem('adress') as HTMLInputElement;
    ...
    this.orderButtonElement.addEventListener('click', (evt) => {
			evt.preventDefault();
			events.emit('order:submit', { address: this.inputAddress.value, payment: this.payment });
		});
